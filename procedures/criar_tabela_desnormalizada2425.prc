...
BEGIN
  EXECUTE IMMEDIATE q'[
    ...
    FROM "PORTO"."ORDERS"@"CLOUD$LINK" o
    JOIN "PORTO"."CUSTOMERS"@"CLOUD$LINK" c
        ON o.CUSTOMER_ID = c.CUSTOMER_ID
    LEFT JOIN "PORTO"."ADDRESSES"@"CLOUD$LINK" a
        ON o.DELIVERY_ADDRESS_ID = a.ADDRESS_ID
    LEFT JOIN "PORTO"."CARD_DETAILS"@"CLOUD$LINK" cd
        ON o.CARD_ID = cd.CARD_ID
    JOIN "PORTO"."ORDER_ITEMS"@"CLOUD$LINK" oi
        ON o.ORDER_ID = oi.ORDER_ID
    LEFT JOIN "PORTO"."PRODUCT_INFORMATION"@"CLOUD$LINK" pi
        ON oi.PRODUCT_ID = pi.PRODUCT_ID
    LEFT JOIN "PORTO"."PRODUCT_DESCRIPTIONS"@"CLOUD$LINK" pd
        ON pd.PRODUCT_ID = oi.PRODUCT_ID
        AND pd.LANGUAGE_ID = 'PT'
    LEFT JOIN "PORTO"."INVENTORIES"@"CLOUD$LINK" inv
        ON oi.PRODUCT_ID = inv.PRODUCT_ID
        AND o.WAREHOUSE_ID = inv.WAREHOUSE_ID
    LEFT JOIN "PORTO"."WAREHOUSES"@"CLOUD$LINK" w
